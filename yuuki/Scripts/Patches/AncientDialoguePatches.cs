...
public static class AncientDialoguePatches
{
	...
	{
		if (!__result.CharacterDialogues.ContainsKey("YUUKI-YUKI_CHARACTER"))
		{
			List<AncientDialogue> list = new List<AncientDialogue>();
			for (int i = 0; i < visitCount; i++)
			{
				list.Add(new AncientDialogue($"{npcName}.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-0.ancient", sfx)
				{
					VisitIndex = i
				});
				list.Add(new AncientDialogue($"{npcName}.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-1.char", "")
				{
					VisitIndex = i,
					IsRepeating = (i > 0)
				});
			}
			__result.CharacterDialogues["YUUKI-YUKI_CHARACTER"] = list;
		}
	}
	...
	public static void ArchitectPostfix(ref AncientDialogueSet __result)
	{
		if (!__result.CharacterDialogues.ContainsKey("YUUKI-YUKI_CHARACTER"))
		{
			...
			for (int i = 0; i < 3; i++)
			{
				list.Add(new AncientDialogue($"THE_ARCHITECT.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-0r.ancient", "")
				{
					VisitIndex = i,
					EndAttackers = ArchitectAttackers.Both
				});
				list.Add(new AncientDialogue($"THE_ARCHITECT.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-1r.char", "")
				{
					VisitIndex = i,
					EndAttackers = ArchitectAttackers.Both
				});
				list.Add(new AncientDialogue($"THE_ARCHITECT.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-2r.ancient", "")
				{
					VisitIndex = i,
					EndAttackers = ArchitectAttackers.Both
				});
				list.Add(new AncientDialogue($"THE_ARCHITECT.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-3r.char", "")
				{
					VisitIndex = i,
					EndAttackers = ArchitectAttackers.Both
				});
				list.Add(new AncientDialogue($"THE_ARCHITECT.talk.{"YUUKI-YUKI_CHARACTER"}.{i}-4r.ancient", "")
				{
					VisitIndex = i,
					EndAttackers = ArchitectAttackers.Both
				});
