...
public override IEnumerable<CardKeyword> CanonicalKeywords => [(CardKeyword)1];
	...
	{
		List<CardModel> list = PileTypeExtensions.GetPile((PileType)2, this.Owner).Cards.Where((CardModel c) => c != null && (object)c != this).ToList();
		if (list.Count > 0)
		{
			...
			{
				List<Creature> list2 = this.CombatState.Enemies.Where((Creature e) => e.IsAlive).ToList();
				if (list2.Count > 0)
				{
					val2 = list2[this.Owner.RunState.Rng.Shuffle.NextInt(0, list2.Count)];
				}
			}
			await CardCmd.AutoPlay(choiceContext, val, val2, (AutoPlayType)1, false, false);
		}
